...
export const veloraPlugins = Object.freeze(["legacy-disclosure", "dropdown", "collapse", "modal", "tabs", "toast", "tooltip", "offcanvas"]);
...
function createController(root, enabledPlugins) {
  ...
  const offcanvasReturnFocus = new WeakMap();
  ...
  function resolveTooltipPlacement(trigger, target) {
    return (
      trigger.getAttribute("data-vel-tooltip-placement")
      || target.getAttribute("data-vel-tooltip-placement")
      || target.dataset.velPlacement
      || "top"
    );
  }
  ...
  function resolveOffcanvasPlacement(trigger, target) {
    const placement = trigger?.getAttribute("data-vel-offcanvas-placement")
      || target.getAttribute("data-vel-offcanvas-placement")
      || target.dataset.velPlacement
      || offcanvasPlacements.find((candidate) => target.classList.contains(`vel-offcanvas-${candidate}`))
      || "end";
  ...
  function getAccordionGroup(trigger, target) {
    return trigger?.closest?.(".vel-accordion") ?? target?.closest?.(".vel-accordion") ?? null;
  }

  function getAccordionPanels(group) {
    return queryAll(group, ".vel-collapse, [data-vel-collapse-panel]").filter((panel) => containsNode(group, panel));
  }

  function openCollapse(target, trigger = null) {
    if (!target || !pluginEnabled("collapse")) {
      return;
    }

    const accordionGroup = getAccordionGroup(trigger, target);

    if (accordionGroup) {
      getAccordionPanels(accordionGroup).forEach((panel) => {
        if (panel !== target && !isHidden(panel)) {
          hideWithState(root, panel);
        }
      });
    }
  ...
  function toggleCollapse(target, trigger = null) {
    if (!target || !pluginEnabled("collapse")) {
      return;
    }

    if (isHidden(target)) {
      openCollapse(target, trigger);
      return;
    }
  ...
  function handleKeydown(event) {
    ...
    const openOffcanvas = pluginEnabled("offcanvas") ? getVisible(".vel-offcanvas") : [];

    if (openOffcanvas.length > 0) {
      closeOffcanvas(openOffcanvas[openOffcanvas.length - 1]);
      return;
    }
  ...
  function handleClick(event) {
    ...
    if (collapseTrigger && pluginEnabled("collapse")) {
      event.preventDefault();
      toggleCollapse(resolveTriggerTarget(root, collapseTrigger, "data-vel-collapse"), collapseTrigger);
      return;
    }
    ...
    if (legacyOpenTrigger && pluginEnabled("legacy-disclosure")) {
      ...
      if (target) {
        if (isCollapseTarget(target) && pluginEnabled("collapse")) {
          openCollapse(target, legacyOpenTrigger);
          return;
        }
    ...
    if (legacyToggleTrigger && pluginEnabled("legacy-disclosure")) {
      ...
      if (isCollapseTarget(target) && pluginEnabled("collapse")) {
        toggleCollapse(target, legacyToggleTrigger);
        return;
      }
...
export function createVeloraRuntime({ plugins = veloraPlugins } = {}) {
...
const defaultRuntime = createVeloraRuntime();

export function initVelora(root = document, options = {}) {
  if (options.plugins) {
